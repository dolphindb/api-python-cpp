...
#define DLOGPRINT //ddb::DLogger::Info

std::string GetObjectStr(PyObject *obj);
#define GETPOS() (in_->getPosition() - 1 - (frameIdx_ < frameLen_ ? frameLen_ - frameIdx_ : 0))
#define DLOGOBJ(text, obj) DLOGPRINT(text,"pos",GETPOS(),"obj",GetObjectStr(obj));
#define DLOG2(text, param) DLOGPRINT(text,param);
#define DLOGOP(text, op) DLOGPRINT(text,"pos",GETPOS(),"op",char(op),0xff&op,"stack",Py_SIZE(unpickler_->stack) - unpickler_->stack->fence);
            ...
            while (get_opr(op, ret))
            {
                if (!do_opr(op, ret)){
                    if(op != Pickle::opcode::STOP){
                        DLogger::Error("do operation",op&0xff,char(op),"failed at filepos",GETPOS());
                    }
                    break;
                }
            }
        }catch(...){
            DLogger::Error("do operation",op&0xff,char(op),"exception at filepos",GETPOS());
        }
        if (ret != OK)
        {
