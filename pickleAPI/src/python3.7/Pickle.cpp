...
namespace dolphindb{
    ...
    int PickleUnmarshall::load_counted_binbytes(size_t nbytes, IO_ERR &ret) {
        ...
        if (frameLen_ - frameIdx_ < nbytes) {
            if((ret = in_->readBytes(shortBuf_, nbytes, false)) != OK){
                //printf("load_counted_binbytes readBytes1 failed %d.",ret);
                return -1;
            }
            *(&s) = shortBuf_;
        } else {
        ...
        if (size < 0) {
            PyErr_Format(PyExc_OverflowError,
                         ...
                         PY_SSIZE_T_MAX);
            //printf("load_counted_binbytes calc_binsize failed %d.",size);
            return -1;
        }
        ...
        if (frameLen_ - frameIdx_ < size) {
            ...
            if (op == NULL){
                //printf("load_counted_binbytes _PyBytes_FromSize failed %d.",size);
                return -1;
            }
            size_t begIdx=0, actualSize=0;
            const size_t BUFFSIZE = 65536;
            while(begIdx < size) {
                ...
                if((ret = in_->readBytes(op->ob_sval+begIdx, actualSize, actualSize)) != OK){
                    //printf("load_counted_binbytes readBytes2 failed %d.",ret);
                    return -1;
                }
                begIdx += actualSize;
            }
    ...
    bool PickleUnmarshall::start(short flag, bool blocking, IO_ERR &ret) {
        //printf("PickleUnmarshall::start2");
        if((ret = in_->readBytes(shortBuf_, 2, false)) != OK){
            ret = NODATA;
        ...
        if(shortBuf_[0] != Pickle::opcode::PROTO) {
            ret = INVALIDDATA;
            //printf("Header value mismatch %d.",shortBuf_[0]);
            return false;
        }
        int i = (unsigned char)shortBuf_[1];
        if(i >  HIGHEST_PROTOCOL) {
            ret = INVALIDDATA;
            //printf("Protocol version mismatch  %d.",i);
            return false;
        }
        unpickler_->proto = i;
        ...
        while (1) {
            ...
            if(frameIdx_ < frameLen_ ){
                ...
                if((ret = in_->readChar(op)) != OK){
                    ret = NODATA;
                    //printf("Read continue flag error.");
                    return false;
                }
            }
            switch ((enum Pickle::opcode)op) {
                ...
                default:
                {
                    ...
                    //printf("Read invalid key %d.",c);
                    ret = INVALIDDATA;
                    return false;
                }
            }
            //printf("Read invalid op %d.",op);
            break;                  /* and we are done! */
        }
        if(ret != OK) {
            //printf("Read object result error %d.",ret);
            return false;
        }
        if (PyErr_Occurred()) {
            //printf("Read object PyErr_Occurred");
            return false;
        }
        if (_Unpickler_SkipConsumed(unpickler_) < 0){
            //printf("Read object _Unpickler_SkipConsumed");
            return false;
        }
    ...
    void PickleUnmarshall::reset() {
        //std::cout << "begin to reset" << std::endl;
        ...
        //std::cout << "use time = " << (t2.tv_sec-t1.tv_sec)*1000000 + t2.tv_usec-t1.tv_usec << std::endl;
    }
};
